            ...
            <thead>
                <tr style="background-color: darkgray;">
                    @if (Session::has(appGlobals::getInfoMessageType()))
                        <th id="thAlertMessage" colspan="9" class="text-center">
                            <div class="tbl-h1-height">
                                <span style="color: brown;font-weight: bold">{{ Session::get(appGlobals::getInfoMessageType()) }}</span>
                            </div>
                        </th>
                        <th id="thNoAlertMessage" colspan="9" class="text-center" style="display: none">
                            <div class="tbl-h1-height">
                                @include('partials.calenderControl')
                            </div>
                        </th>
                    @else
                ...
                <tbody id="timeCardTable">
                    @foreach ($timeCardRows as $timeCardRow)
                        <tr>
                            ...
                            <td>
                                @if ($timeCardRow->dow == $timeCardRow->timeCardFormat->dow_00)
                                    {!! link_to_route('task.show', $timeCardRow->hours_worked, $timeCardRow->id) !!}
                                @else
                                    0
                                @endif
                            </td>
                            <td>
                                @if ($timeCardRow->dow == $timeCardRow->timeCardFormat->dow_01)
                                    {!! link_to_route('task.show', $timeCardRow->hours_worked, $timeCardRow->id) !!}
                                @else
                                    0
                                @endif
                            ...
                            <td>
                                @if ($timeCardRow->dow == $timeCardRow->timeCardFormat->dow_02)
                                    {!! link_to_route('task.show', $timeCardRow->hours_worked, $timeCardRow->id) !!}
                                @else
                                    0
                                @endif
                            ...
                            <td>
                                @if ($timeCardRow->dow == $timeCardRow->timeCardFormat->dow_03)
                                    {!! link_to_route('task.show', $timeCardRow->hours_worked, $timeCardRow->id) !!}
                                @else
                                    0
                                @endif
                            ...
                            <td>
                                @if ($timeCardRow->dow == $timeCardRow->timeCardFormat->dow_04)
                                    {!! link_to_route('task.show', $timeCardRow->hours_worked, $timeCardRow->id) !!}
                                @else
                                    0
                                @endif
                            ...
                            <td>
                                @if ($timeCardRow->dow == $timeCardRow->timeCardFormat->dow_05)
                                    {!! link_to_route('task.show', $timeCardRow->hours_worked, $timeCardRow->id) !!}
                                @else
                                    0
                                @endif
                            ...
                            <td>
                                @if ($timeCardRow->dow == $timeCardRow->timeCardFormat->dow_06)
                                    {!! link_to_route('task.show', $timeCardRow->hours_worked, $timeCardRow->id) !!}
                                @else
                                    0
                                @endif
